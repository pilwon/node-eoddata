      ...
      request.post({
        ...
        if (res.statusCode === 200) {
          xml2js.parseString(body, function (err, result) {
            if (err) { return cb(err); }
            if (!result.RESPONSE.QUOTES) {
              return cb(new Error(util.format('Failed to get QuoteListByDate (%s, %s): %s',
                                              exchangeCode,
          ...
          return cb(new Error(util.format('Failed to get QuoteListByDate (%s, %s): status %s',
                                          exchangeCode,
                                          moment(quoteDate).format('M/D/YYYY'),
                                          res.statusCode)));
        }
        ...
        if (res.statusCode === 200) {
          xml2js.parseString(body, function (err, result) {
            ...
            if (!result.RESPONSE.QUOTES2) {
              return cb(new Error(util.format('Failed to get QuoteListByDate2 (%s, %s): %s',
                                              exchangeCode,
                                              moment(quoteDate).format('M/D/YYYY'),
                                              result.RESPONSE.$.Message)));
            }
          ...
          return cb(new Error(util.format('Failed to get QuoteListByDate2 (%s, %s): status %s',
                                          exchangeCode,
                                          moment(quoteDate).format('M/D/YYYY'),
                                          res.statusCode)));
        }
      ...
      request.post({
        ...
        if (res.statusCode === 200) {
          xml2js.parseString(body, function (err, result) {
            if (err) { return cb(err); }
            if (!result.RESPONSE.QUOTES) {
              return cb(new Error(util.format('Failed to get QuoteListByDatePeriod (%s, %s, %s): %s',
                                              exchangeCode,
                                              moment(quoteDate).format('M/D/YYYY'),
                                              period,
          ...
          return cb(new Error(util.format('Failed to get QuoteListByDatePeriod (%s, %s, %s): status %s',
                              exchangeCode,
                              moment(quoteDate).format('M/D/YYYY'),
                              period,
                              res.statusCode)));
      ...
      request.post({
        ...
        if (res.statusCode === 200) {
          xml2js.parseString(body, function (err, result) {
            if (err) { return cb(err); }
            if (!result.RESPONSE.QUOTES2) {
              return cb(new Error(util.format('Failed to get QuoteListByDatePeriod2 (%s, %s, %s): %s',
                                              exchangeCode,
                                              moment(quoteDate).format('M/D/YYYY'),
                                              period,
                                              result.RESPONSE.$.Message).bold));
          ...
          return cb(new Error(util.format('Failed to get QuoteListByDatePeriod2 (%s, %s, %s): status %s',
                              exchangeCode,
                              moment(quoteDate).format('M/D/YYYY'),
                              period,
                              res.statusCode)));
        ...
        if (res.statusCode === 200) {
          xml2js.parseString(body, function (err, result) {
            ...
            if (!result.RESPONSE.SPLITS) {
              return cb(new Error(util.format('Failed to get SplitListByExchange (%s): %s',
                                              exchangeCode, result.RESPONSE.$.Message).bold));
            }
            var splits = [];
          ...
          return cb(new Error(util.format('Failed to get SplitListByExchange (%s): status %s',
                              exchangeCode, res.statusCode)));
        }
      });
    });
      ...
      request.post({
        ...
        if (res.statusCode === 200) {
          xml2js.parseString(body, function (err, result) {
            if (err) { return cb(err); }
            if (!result.RESPONSE.SPLITS) {
              return cb(new Error(util.format('Failed to get SplitListBySymbol (%s:%s): %s',
                                              exchangeCode, symbolCode, result.RESPONSE.$.Message).bold));
            }
            var split = result.RESPONSE.SPLITS[0].SPLIT[0].$;
            debug(util.format('SplitListBySymbol (%s:%s)', exchangeCode, symbolCode).yellow);
            return cb(null, split);
          });
        } else {
          return cb(new Error(util.format('Failed to get SplitListBySymbol (%s:%s): status %s',
                              exchangeCode, symbolCode, res.statusCode)));
        }
      });
    });
      ...
      request.post({
        ...
        if (res.statusCode === 200) {
          xml2js.parseString(body, function (err, result) {
            if (err) { return cb(err); }
            if (!result.RESPONSE.QUOTES) {
              return cb(new Error(util.format('Failed to get Top10Gains (%s): %s',
                                              exchangeCode,
                                              result.RESPONSE.$.Message).bold));
            }
            var quotes = [];
          ...
          return cb(new Error(util.format('Failed to get Top10Gains (%s): status %s',
                              exchangeCode,
                              res.statusCode)));
        }
      });
      ...
      request.post({
        ...
        if (res.statusCode === 200) {
          xml2js.parseString(body, function (err, result) {
            if (err) { return cb(err); }
            if (!result.RESPONSE.QUOTES) {
              return cb(new Error(util.format('Failed to get Top10Losses (%s): %s',
                                              exchangeCode,
                                              result.RESPONSE.$.Message).bold));
            }
            var quotes = [];
          ...
          return cb(new Error(util.format('Failed to get Top10Losses (%s): status %s',
                              exchangeCode,
                              res.statusCode)));
        }
      });
